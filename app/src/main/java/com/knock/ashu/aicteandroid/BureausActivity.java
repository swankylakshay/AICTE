...
public class BureausActivity extends ActionBarActivity implements ViewAnimator.ViewAnimatorListener {
    ...
    private BureausFragment bureausFragment;

    public static Intent newIntent(Context context) {
        return new Intent(context, BureausActivity.class);
    }

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_bureaus);
        bureausFragment = BureausFragment.newInstance();
        getSupportFragmentManager().beginTransaction()
                .replace(R.id.content_frame, bureausFragment)
                .commit();
        ...
        setActionBar();
        createMenuList();
        viewAnimator = new ViewAnimator<>(this, list, bureausFragment, drawerLayout, this);
    }
    ...
    private ScreenShotable replaceFragment(ScreenShotable screenShotable, int topPosition) {
        ...
        animator.start();
        if (topPosition == 0) {
            BureausFragment bureausFragment = BureausFragment.newInstance();
            getSupportFragmentManager().beginTransaction().replace(R.id.content_frame, bureausFragment).commit();
            return bureausFragment;
        }
        else
            return null;
    }
